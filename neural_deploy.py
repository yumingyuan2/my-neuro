...
def install_dependencies():
    ...
    if os.path.exists("jieba_fast-0.53-cp311-cp311-win_amd64.whl"):
        run_in_conda_env(
            "pip install jieba_fast-0.53-cp311-cp311-win_amd64.whl -i https://mirrors.tuna.tsinghua.edu.cn/pypi/web/simple")
    else:
...
def download_tts_models():
    ...
    if not download_with_retry(
            "call conda activate my-neuro && modelscope download --model morelle/Omni_fn_bert --local_dir ./"):
    ...
    if not download_with_retry(
            "call conda activate my-neuro && modelscope download --model zxm2493188292/G2PWModel --local_dir ./"):
    ...
    if not download_with_retry(
            "call conda activate my-neuro && modelscope download --model AI-ModelScope/GPT-SoVITS --local_dir ./tts-studio/GPT_SoVITS/pretrained_models"):
    ...
    download_success = download_with_retry(
        "call conda activate my-neuro && modelscope download --model morelle/Fake-Neuro-TTS-V2 --local_dir ./")
    ...
    if not download_with_retry(
            "call conda activate my-neuro && modelscope download --model BAAI/bge-m3 --local_dir ./RAG-model"):
    ...
    if not download_with_retry(
            f"call conda activate my-neuro && modelscope download --model AI-ModelScope/uvr5_weights HP2_all_vocals.pth --local_dir {uvr5_weights_dir}"):
        print("UVR5权重文件下载失败")
        # 不终止程序，因为这是额外的模型
    else:
        print("UVR5权重文件下载成功！")
    ...
    if not download_with_retry(
            f"call conda activate my-neuro && modelscope download --model pengzhendong/faster-whisper-medium --local_dir {faster_whisper_dir}"):
    ...
    if not download_with_retry(
            "call conda activate my-neuro && modelscope download --model morelle/nltk_data --local_dir ./tts-studio"):
        print("nltk_data下载失败")
        # 不终止程序，因为这是额外的数据
    else:
        print("nltk_data下载成功！")
