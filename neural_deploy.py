...
def run_in_conda_env(command):
    ...
    return subprocess.run(conda_command, shell=shell)
...
# 5. 下载G2PWModel到tts-studio/text文件夹
# 返回到原始目录
os.chdir(current_dir)
...
# 6. 复制G2PWModel到tts-studio/GPT_SoVITS/text文件夹
# 返回到原始目录
os.chdir(current_dir)
...
# 7. 下载并解压pretrained_models.zip到tts-studio/GPT_SoVITS/pretrained_models文件夹
# 创建目标目录
...
# 8. 复制预训练模型到tts-studio/pretrained_models文件夹
# 确保我们在当前工作目录
os.chdir(current_dir)
